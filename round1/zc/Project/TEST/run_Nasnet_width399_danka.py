...
fine_result_path = './fine_result_b/NASNET/Width399/'
...
def predict_result(stage_flag,df_load,X_test,batch_size=5):
 ...
 #    test_np = model.predict(X_test, batch_size=8)
  ...
  if(stage_flag==1):
    df_load.to_csv(fine_result_path + '{}_danka_stage1.csv'.format(prefix_cls), header=None, index=False)
  elif(stage_flag==2):
    df_load.to_csv(fine_result_path + '{}_danka_stage2.csv'.format(prefix_cls), header=None, index=False)
