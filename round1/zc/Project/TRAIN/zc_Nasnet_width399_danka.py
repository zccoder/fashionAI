...
#test_anno_path = '/home2/data/fashionAI/z_rank/Tests/question.csv'
#test_pict_path = '/home2/data/fashionAI/z_rank'

model_fine_result = './fine_result/NASNET/'
model_path = './models/NASNET/ADAM/'
...
def fit_aug_model(model,epochs=7,train_batch_size=5,valid_batch_size=5):
  ...
  checkpointer = ModelCheckpoint(filepath='./models/NASNET/Width/{0}_danka.best.h5'.format(prefix_cls), verbose=1,save_best_only=True)
  ...
  h=model.fit_generator(datagen.flow(X_train, y_train, batch_size=train_batch_size), epochs=epochs,
                      ...
                      callbacks=[EarlyStopping(patience=5), CustomModelCheckpoint(model, './models/NASNET/Width/{0}_danka.best.npy'.format(prefix_cls))])
...
def Read_test_picture(df_load,width,test_pict_path):
  ...
  for i in range(n):
    X_test[i] = cv2.resize(cv2.imread('/home2/data/fashionAI/test_data/{0}'.format(df_load['image_id'][i])), (width, width))
  return n,X_test
...
def predict_result(stage_flag,df_load,X_test,batch_size=5):
 ...
 #    test_np = model.predict(X_test, batch_size=8)
  ...
  if(stage_flag==1):
    df_load.to_csv('./fine_result/NASNET/Width/{}_danka_stage1.csv'.format(prefix_cls), header=None, index=False)
  elif(stage_flag==2):
    df_load.to_csv('./fine_result/NASNET/Width/{}_danka_stage2.csv'.format(prefix_cls), header=None, index=False)
...
weight = np.load('./models/NASNET/Width/{0}_danka.best.npy'.format(prefix_cls))
model.set_weights(weight)

# model.load_weights('./models/NASNET/SGD/{0}.best.npy'.format(prefix_cls))
...
#step5
print ('Begin 2 fine tuning ...')
#if index_ == 0:
#   model=fit_fine_tuning(model,cnn_model,top_n_layer=-3,fine_epoches=50,fine_batch_size=20) # for idx 0
#else:
#   model=fit_fine_tuning(model,cnn_model,top_n_layer=-3,fine_epoches=50,fine_batch_size=7)
model=fit_fine_tuning(model,cnn_model,top_n_layer=-3,fine_epoches=15,fine_batch_size=bs)
weight = np.load('./models/NASNET/Width/{0}_danka.best.npy'.format(prefix_cls))
model.set_weights(weight)
# model.load_weights('./models/NASNET/SGD/{0}.best.npy'.format(prefix_cls))
print(model.evaluate(X_valid, y_valid, batch_size=bs))
...
#np.save('./fine_model_backup/NASNET/%s_nas_45.npy'%prefix_cls, weight)
# model.save('./fine_model_backup/NASNET/%s_nas_sgd_0328.h5'%prefix_cls)
