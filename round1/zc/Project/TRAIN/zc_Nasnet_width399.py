...
def fit_aug_model(model,epochs=7,train_batch_size=5,valid_batch_size=5):
  ...
  checkpointer = ModelCheckpoint(filepath='./models/NASNET/Width/{0}_399.best.h5'.format(prefix_cls), verbose=1,save_best_only=True)
  ...
  h=model.fit_generator(datagen.flow(X_train, y_train, batch_size=train_batch_size), epochs=epochs,
                      ...
                      callbacks=[EarlyStopping(patience=5), CustomModelCheckpoint(model, './models/NASNET/Width/{0}_399.best.npy'.format(prefix_cls))])
...
def predict_result(stage_flag,df_load,X_test,batch_size=5):
 ...
 #    test_np = model.predict(X_test, batch_size=8)
  ...
  if(stage_flag==1):
    df_load.to_csv('./fine_result/NASNET/Width/{}_399_stage1.csv'.format(prefix_cls), header=None, index=False)
  elif(stage_flag==2):
    df_load.to_csv('./fine_result/NASNET/Width/{}_399_stage2.csv'.format(prefix_cls), header=None, index=False)
...
weight = np.load('./models/NASNET/Width/{0}_399.best.npy'.format(prefix_cls))
